---
title: Introduction
description: DubStack is a CLI for stacked diffs — dependent git branches that make code review faster and more focused.
---
...
- **Safe merges and retargeting** — `dub merge-next` picks the next safe PR, `dub post-merge` repairs metadata after manual merges, and the [`wiseiodev/dubstack-retarget`](https://github.com/wiseiodev/dubstack-retarget) GitHub Action retargets dependent PR bases automatically.
...
- **Shell and IDE integration** — `dub status` for prompts, `dub completion <shell>` for shell completions, `dub man` for a generated man page, themable terminal output.

## Prerequisites

- **`git` installed** — Version control must be available on your system.
- **`gh` CLI authenticated** — Run `gh auth login` to authenticate.
- **`dub` installed** — See [Installation](/docs/getting-started/installation) for details.
