...
6. On rebase conflict, prompts with the same three-option dialog used by
   `dub restack`:
   - **Continue resolving** — leaves files in their conflicted state. The
     conflict happened inside the reorder rebase, so no `restack-progress.json`
     exists; resolve files, then run `git rebase --continue`, then
     `dub restack` to rebase descendants.
   ...
   If instead the cascading restack of descendants hits a conflict (after the
   reorder itself completed cleanly), the standard `dub continue` /
   `dub abort` recovery applies, because that path *did* write a
...
- `dub undo` — roll back the last create/restack/move/rename/reorder.
