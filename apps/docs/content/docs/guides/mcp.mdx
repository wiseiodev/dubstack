...
The Tier 3 command family is still landing incrementally. `dubstack.freeze`,
`dubstack.unfreeze`, `dubstack.back`, and `dubstack.reorder` will be added to
the MCP surface when their underlying CLI commands land.
