...
- **Reviewers.** `spr config.reviewers` is not migrated. Run `dub config reviewers alice,@org/team`.
...
# 10. Remove spr's pre-commit hook
git config --unset commit.template || true
rm -f .git/hooks/commit-msg     # if it was an spr hook only
```
