...
| `ghstack rage` | `dub doctor` | Diagnostic for misconfigured state |
| `ghstack checkout $URL` | `dub co <branch>` | Charles will name the branch; pick it from the picker |
| `ghstack hash` (n/a) | `dub info` | Stack-aware branch metadata |
